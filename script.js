...
function editGoal(goalIndex) {
    ...
    // Preencher jogadores do time que marcou o gol
    ...
    // Preencher assistências (excluindo o marcador do gol)
    scoringTeam.players.forEach((player, index) => {
...
function updateAssistOptions() {
    ...
    // Se não há jogador selecionado para o gol, não preencher assistências
...
function saveGoal() {
    ...
    const scorer = scoringTeam.players[scorerIndex];
    const assist = assistIndex !== null ? scoringTeam.players[assistIndex] : null;
    ...
    // Verificar se a partida terminou (2 gols do mesmo time)
    if (currentMatch.scoreA === 4 || currentMatch.scoreB === 4) {
        endMatch();
    } else {
...
function updateAllStats() {
    ...
    if (topScorers.length === 0) {
        ...
        topScorers.forEach((player, index) => {
            ...
            playerRank.textContent = `${index + 1}º`;
